...
test.for = ({extend_object}) => extend_object;

test.case("no params", (assert, extend_object) => {
  assert(extend_object()).equals({});
});

test.case("no base", (assert, extend_object) => {
  const extension = {"key": "value"};
  assert(extend_object(undefined, extension)).equals(extension);
});

test.case("no extension", (assert, extend_object) => {
  const base = {"keys": "values"};
  assert(extend_object(base)).equals(base);
});

test.case("base and extension same", (assert, extend_object) => {
  const object = {"key": "value"};
  assert(extend_object(object, object)).equals(object);
});

test.case("one property", (assert, extend_object) => {
  const base = {"key": "value"};
  const extension = {"key": "value2"};
  assert(extend_object(base, extension)).equals(extension);
});

test.case("two properties, one replaced",(assert, extend_object) => {
...
test.case("arrays overwritten", (assert, extend_object) => {
  const base = {"key": ["value", "value2"]};
  const extension = {"key": ["value3", "value4"]};
  assert(extend_object(base, extension)).equals(extension);
});

test.case("one property of a subobject", (assert, extend_object) => {
  const base = {"key": {"subkey": "subvalue"}};
  const extension = {"key": {"subkey": "subvalue 2"}};
  assert(extend_object(base, extension)).equals(extension);
});

test.case("two properties of a subobject, one replaced",
  (assert, extend_object) => {
...
test.case("configuration enhancement", (assert, extend_object) => {
  const default_conf = {
    "base": "/",
    "debug": false,
    "defaults": {
