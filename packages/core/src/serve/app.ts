...
import type { Handler, Options as HandlerOptions } from "#handler";
import type { Mode } from "#mode";
import { s_server } from "#symbols";
import crypto from "@rcompat/crypto";
import file from "@rcompat/fs/file";
import join from "@rcompat/fs/join";
import type Router from "@rcompat/fs/router";
...
const nctag = (name: string, properties: R) =>
...
export interface RuntimeApp extends BaseApp {
  hash: typeof hash,
  secure: boolean,
  assets: Options["assets"],
  files: R,
  get_component(name: string): unknown,
  handlers: RecordMaybe<Handler>,
  headers(csp?: object): object,
  asset_csp: CSP,
  render(content: RenderOptions): string,
  // todo: change to `typeof import("primate/loader")`
  loader: {
    page(page: unknown): string,
  },
  respond(...args: ConstructorParameters<typeof Response>): Response,
  view(options: ResponseInit & RenderOptions): Promise<Response>,
  media(type: string, options?: HandlerOptions): HandlerOptions,
  ...
  register(extension: string, handler: Handler): undefined;
  build_target: string,
  stop(): undefined,
  mode: Mode,
  types: unknown,
  router: ReturnType<typeof Router.init>;
  route(request: Request): Record<string, unknown>,
}

export default async (rootfile: string, build: Options) => {
  ...
  return {
    ...
    media(type, { status = Status.OK, headers } = {}) {
    ...
    register(extension, handler) {
      if (this.handlers[extension] !== undefined) {
        double_extension(extension);
      }
      this.handlers[extension] = handler;
    },
    stop() {
      (this.get(s_server) as { stop: () => undefined }).stop();
    },
    // noops
    target(_, _1) {},
    bind(_, _1) {},
  } as const satisfies RuntimeApp;
};
